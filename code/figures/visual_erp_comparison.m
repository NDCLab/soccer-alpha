function visual_erp_comparison(filenames, folder,  varargin)
% visual_comperp() - Plots ERP waves for comparison.
%
% Usage: 
%   >> visual_erp_comparison(filenames, folder, optionalParameters)
% 
%   Mandatory Inputs:
...
if ~isempty(cluster)
   ...
   channels = ei_channelNameToNumber(cluster); 
   ...
   data(iC).chanlocs(i).labels = cell2mat(cluster);
   end
